...
def _fetch(ticker, period, interval):
    """Single-ticker fetch using Ticker.history() — no MultiIndex, no bleeds."""
    try:
        t  = yf.Ticker(ticker)
...
def fetch_data(ticker, period="1y", interval="1d"):
    return _fetch(ticker, period, interval)


def fetch_weekly(ticker):
    return _fetch(ticker, "2y", "1wk")


def fetch_monthly(ticker):
    return _fetch(ticker, "5y", "1mo")


def fetch_4h(ticker):
    """Yahoo Finance doesn't support 4H — fetch 1H and resample."""
    df = _fetch(ticker, "60d", "1h")
    if df is None:
        return None
...
def fetch_2h(ticker):
    """Yahoo Finance doesn't support 2H — fetch 1H and resample."""
    df = _fetch(ticker, "60d", "1h")
    if df is None:
        return None
