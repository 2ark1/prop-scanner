...
# ── Data Fetching ───────────────────────────────────────────────────────────────
def _clean_df(df, ticker):
    """Flatten MultiIndex columns and ensure correct ticker data is returned."""
    if df is None or df.empty:
        return None
    if isinstance(df.columns, pd.MultiIndex):
        # MultiIndex: (field, ticker) — select only our ticker's columns
