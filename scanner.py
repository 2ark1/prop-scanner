...
def detect_bos_tfcot(df_4h, df_2h):
    ...
    return bos_direction, bos_detail, tfcot_detail, bos_swing_low, bos_swing_high
...
def compute_avwap(df, anchor_price=None, anchor_bars=20):
    ...
    if anchor_price is not None:
        # Find the bar whose Low (for bullish swing low) or High (for bearish swing high)
        # is closest to the BOS anchor price — anchor from that bar forward
        mid = (df['High'] + df['Low']) / 2
        anchor_pos = int((mid - anchor_price).abs().argmin())
    else:
...
def scan_instrument(symbol, info):
    ...
    bos_direction, bos_detail, tfcot_detail, bos_swing_low, bos_swing_high = detect_bos_tfcot(df_4h, df_2h)
    ...
    # Precision — use actual BOS swing if available, else fall back to 20-day range
    if bos_swing_low and bos_swing_high and bos_swing_high > bos_swing_low:
    ...
    # Anchor AVWAP from the BOS swing point (swing low for bullish, swing high for bearish)
    if bos_direction == "Bullish" and bos_swing_low:
        avwap_anchor = bos_swing_low
    elif bos_direction == "Bearish" and bos_swing_high:
        avwap_anchor = bos_swing_high
    else:
        avwap_anchor = None
    avwap = compute_avwap(df_daily, anchor_price=avwap_anchor)
    poc   = compute_fixed_vol_poc(df_daily, bins=30)
    result["avwap"] = avwap
    result["poc"]   = poc
